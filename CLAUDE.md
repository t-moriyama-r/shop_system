...
- 設計（Plan Mode の Plan フェーズ）は `.claude/agents/Plan.md` により `fable` に固定している。設計品質を優先するフェーズのため、実行系より上位のモデルを使う。
- 実行系サブエージェント（`coder`, `reviewer`）は `.claude/agents/coder.md` / `reviewer.md` により `sonnet` に固定している。
- 上記はサブエージェント起動時のモデルにのみ効く。トップレベル（対話セッション本体）が使うモデルは Plan Mode の ON/OFF と連動して自動切替されない。計画セッション自体を上位モデルで進めたい場合は、セッション開始時に `/model` 等で手動選択すること。
