...
> バージョン: 2 | 更新日時: 2026/7/2 00:00:00

SE管理画面システムのER図。SE管理者の認証・セッション管理、ショップアカウント発行、操作監査ログ、メール送信ログの5テーブルで構成される。audit_logsのse_admin_user_idはCLI操作時にNULLとなるため厳密な外部キー制約は※要確認。（テーブル定義.mdとの整合を取るため、テーブル名・カラム構成を修正）
